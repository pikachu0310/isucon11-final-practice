...
func (h *handlers) Login(c echo.Context) error {
	...
	var user User
	if err := dbGet(&user, "SELECT * FROM `users` WHERE `code` = ?", req.Code); err != nil && err != sql.ErrNoRows {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
...
func (h *handlers) GetMe(c echo.Context) error {
	...
	var userCode string
	if err := dbGet(&userCode, "SELECT `code` FROM `users` WHERE `id` = ?", userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) GetRegisteredCourses(c echo.Context) error {
	...
	if err := txSelect(tx, &courses, query, StatusClosed, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for _, course := range courses {
		var teacher User
		if err := txGet(tx, &teacher, "SELECT * FROM `users` WHERE `id` = ?", course.TeacherID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func (h *handlers) RegisterCourses(c echo.Context) error {
	...
	for _, courseReq := range req {
		...
		var course Course
		if err := txGet(tx, &course, "SELECT * FROM `courses` WHERE `id` = ? FOR SHARE", courseID); err != nil && err != sql.ErrNoRows {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		} else if err == sql.ErrNoRows {
		...
		// すでに履修登録済みの科目は無視する
		var count int
		if err := txGet(tx, &count, "SELECT COUNT(*) FROM `registrations` WHERE `course_id` = ? AND `user_id` = ?", course.ID, userID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
		if count > 0 {
	...
	query := "SELECT `courses`.*" +
		...
		" JOIN `registrations` ON `courses`.`id` = `registrations`.`course_id`" +
		" WHERE `courses`.`status` != ? AND `registrations`.`user_id` = ?"
	if err := txSelect(tx, &alreadyRegistered, query, StatusClosed, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for _, course := range newlyAdded {
		_, err = txExec(tx, "INSERT INTO `registrations` (`course_id`, `user_id`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `course_id` = VALUES(`course_id`), `user_id` = VALUES(`user_id`)", course.ID, userID)
		if err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func (h *handlers) GetGrades(c echo.Context) error {
	...
	query := "SELECT `courses`.*" +
		...
		" WHERE `user_id` = ?"
	if err := dbSelect(&registeredCourses, query, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for _, course := range registeredCourses {
		...
		query = "SELECT *" +
			...
			" WHERE `course_id` = ?" +
			" ORDER BY `part` DESC"
		if err := dbSelect(&classes, query, course.ID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
		...
		for _, class := range classes {
			...
			if err := dbGet(&submissionsCount, "SELECT COUNT(*) FROM `submissions` WHERE `class_id` = ?", class.ID); err != nil {
				c.Logger().Error(err)
				return c.NoContent(http.StatusInternalServerError)
			}

			var myScore sql.NullInt64
			if err := dbGet(&myScore, "SELECT `submissions`.`score` FROM `submissions` WHERE `user_id` = ? AND `class_id` = ?", userID, class.ID); err != nil && err != sql.ErrNoRows {
				c.Logger().Error(err)
				return c.NoContent(http.StatusInternalServerError)
		...
		query := "SELECT IFNULL(SUM(`submissions`.`score`), 0) AS `total_score`" +
			...
			" WHERE `courses`.`id` = ?" +
			" GROUP BY `users`.`id`"
		if err := dbSelect(&totals, query, course.ID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
	...
	query = "SELECT IFNULL(SUM(`submissions`.`score` * `courses`.`credit`), 0) / 100 / `credits`.`credits` AS `gpa`" +
		...
		" WHERE `users`.`type` = ?" +
		" GROUP BY `users`.`id`"
	if err := dbSelect(&gpas, query, StatusClosed, StatusClosed, Student); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) SearchCourses(c echo.Context) error {
	...
	if err := dbSelect(&res, query+condition, args...); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) AddCourse(c echo.Context) error {
	...
	courseID := newULID()
	_, err = dbExec("INSERT INTO `courses` (`id`, `code`, `type`, `name`, `description`, `credit`, `period`, `day_of_week`, `teacher_id`, `keywords`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
	...
	if err != nil {
		if mysqlErr, ok := err.(*mysql.MySQLError); ok && mysqlErr.Number == uint16(mysqlErrNumDuplicateEntry) {
			var course Course
			if err := dbGet(&course, "SELECT * FROM `courses` WHERE `code` = ?", req.Code); err != nil {
				c.Logger().Error(err)
				return c.NoContent(http.StatusInternalServerError)
			}
...
func (h *handlers) GetCourseDetail(c echo.Context) error {
	...
	query := "SELECT `courses`.*, `users`.`name` AS `teacher`" +
		...
		" WHERE `courses`.`id` = ?"
	if err := dbGet(&res, query, courseID); err != nil && err != sql.ErrNoRows {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	} else if err == sql.ErrNoRows {
...
func (h *handlers) SetCourseStatus(c echo.Context) error {
	...
	var count int
	if err := txGet(tx, &count, "SELECT COUNT(*) FROM `courses` WHERE `id` = ? FOR UPDATE", courseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if count == 0 {
	...
	if _, err := txExec(tx, "UPDATE `courses` SET `status` = ? WHERE `id` = ?", req.Status, courseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) GetClasses(c echo.Context) error {
	...
	var count int
	if err := txGet(tx, &count, "SELECT COUNT(*) FROM `courses` WHERE `id` = ?", courseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if count == 0 {
	...
	if err := txSelect(tx, &classes, query, userID, courseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) AddClass(c echo.Context) error {
	...
	classID := newULID()
	if _, err := txExec(tx, "INSERT INTO `classes` (`id`, `course_id`, `part`, `title`, `description`) VALUES (?, ?, ?, ?, ?)",
		classID, courseID, req.Part, req.Title, req.Description); err != nil {
		_ = tx.Rollback()
		if mysqlErr, ok := err.(*mysql.MySQLError); ok && mysqlErr.Number == uint16(mysqlErrNumDuplicateEntry) {
			var class Class
			if err := dbGet(&class, "SELECT * FROM `classes` WHERE `course_id` = ? AND `part` = ?", courseID, req.Part); err != nil {
				c.Logger().Error(err)
				return c.NoContent(http.StatusInternalServerError)
			}
...
func (h *handlers) RegisterScores(c echo.Context) error {
	...
	for _, score := range req {
		if _, err := txExec(tx, "UPDATE `submissions` JOIN `users` ON `users`.`id` = `submissions`.`user_id` SET `score` = ? WHERE `users`.`code` = ? AND `class_id` = ?", score.Score, score.UserCode, classID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
	}
...
func (h *handlers) DownloadSubmittedAssignments(c echo.Context) error {
	...
	var classCount int
	if err := txGet(tx, &classCount, "SELECT COUNT(*) FROM `classes` WHERE `id` = ? FOR UPDATE", classID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if classCount == 0 {
	...
	query := "SELECT `submissions`.`user_id`, `submissions`.`file_name`, `users`.`code` AS `user_code`" +
		...
		" WHERE `class_id` = ?"
	if err := txSelect(tx, &submissions, query, classID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	if _, err := txExec(tx, "UPDATE `classes` SET `submission_closed` = TRUE WHERE `id` = ?", classID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) GetAnnouncementList(c echo.Context) error {
	...
	if err := txSelect(tx, &announcements, query, args...); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}

	var unreadCount int
	if err := txGet(tx, &unreadCount, "SELECT COUNT(*) FROM `unread_announcements` WHERE `user_id` = ? AND NOT `is_deleted`", userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) AddAnnouncement(c echo.Context) error {
	...
	var count int
	if err := txGet(tx, &count, "SELECT COUNT(*) FROM `courses` WHERE `id` = ?", req.CourseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if count == 0 {
	...
	if _, err := txExec(tx, "INSERT INTO `announcements` (`id`, `course_id`, `title`, `message`) VALUES (?, ?, ?, ?)",
		req.ID, req.CourseID, req.Title, req.Message); err != nil {
		_ = tx.Rollback()
		if mysqlErr, ok := err.(*mysql.MySQLError); ok && mysqlErr.Number == uint16(mysqlErrNumDuplicateEntry) {
			var announcement Announcement
			if err := dbGet(&announcement, "SELECT * FROM `announcements` WHERE `id` = ?", req.ID); err != nil {
				c.Logger().Error(err)
				return c.NoContent(http.StatusInternalServerError)
			}
	...
	if err := txSelect(tx, &targets, query, req.CourseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}

	for _, user := range targets {
		if _, err := txExec(tx, "INSERT INTO `unread_announcements` (`announcement_id`, `user_id`) VALUES (?, ?)", req.ID, user.ID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
	}
...
func (h *handlers) GetAnnouncementDetail(c echo.Context) error {
	...
	if err := txGet(tx, &announcement, query, announcementID, userID); err != nil && err != sql.ErrNoRows {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	} else if err == sql.ErrNoRows {
	...
	var registrationCount int
	if err := txGet(tx, &registrationCount, "SELECT COUNT(*) FROM `registrations` WHERE `course_id` = ? AND `user_id` = ?", announcement.CourseID, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if registrationCount == 0 {
	...
	if _, err := txExec(tx, "UPDATE `unread_announcements` SET `is_deleted` = TRUE WHERE `announcement_id` = ? AND `user_id` = ?", announcementID, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
